...
- *Touch target ergonomics*: `1` (Almost all text links, including critical PDF/Code links in research cards, are under 20px tall and fail the 44px minimum for primary actions. Spacing is tight.)
- *Focus state clarity*: `3` (Standard outlines exist, but lack focus-within considerations and dynamic offset for tight components.)

*Worst skill identified*: **Touch target ergonomics (Skill 5)**. We must harden this skill to strictly require 44x44px tap areas for ALL standalone links (using padding or pseudo-elements) to ensure mobile usability without compromising the minimalist aesthetic.
...
| 4 | Focus states and keyboard clarity | WCAG 2.4.7, 2.4.11 | Yes | Visible, unobscured focus indicator on all interactive controls; focus rings must have >2px offset to avoid clipping; native `outline` preferred over complex box-shadows | Keyboard walkthrough evidence across nav/forms/cards |
| 5 | Touch target ergonomics | WCAG 2.5.8, Apple HIG | Yes | ALL primary actions (nav links, card links, buttons) MUST have a >=44x44px clickable area (via padding/pseudo-elements); inline text links >=24x24px; spacing >8px | Mobile screenshot + CSS evidence of padding/hit-area expansion |
